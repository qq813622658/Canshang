    ...
    handleLogOut({
      ...
      return new Promise((resolve, reject) => {
        logout(state.token).then(() => {
          ...
          // 退出时清除用户权限
          sessionStorage.removeItem("btnaccess");
          commit('setToken', '')
          commit('setAccess', [])
          resolve()
        }).catch(err => {
      ...
      return new Promise((resolve, reject) => {
        try {
          getUserInfo(state.token).then(res => {
            ...
            // 存储按钮权限
            sessionStorage.setItem('btnaccess', data.btnaccess);
            // 存储是否超级管理员权限(admin)
            sessionStorage.setItem('isSuperAdmin', data.roleAccess);
            // 存储是否乡镇权限权限
            sessionStorage.setItem('departType', data.departType);
            // 存储管理员或超级管理员权限（除外的不能正向激励审核）
            sessionStorage.setItem('isManager', data.isManager);
            // 存储departCode
            sessionStorage.setItem('departCode', data.departCode);
